...
public class CompressTools {
    ...
    public void compressToFile(final Bitmap bitmap, final OnCompressListener mOnCompressListener) {
        compressImageJni(bitmap, maxWidth, maxHeight, compressFormat, bitmapConfig, quality, destinationDirectoryPath, fileName, optimize, keepResolution,
                mOnCompressListener);
    }
    ...
    public static void compressImageJni(final Bitmap bitmap, final int maxWidth, final int maxHeight, Bitmap.CompressFormat compressFormat,
        ...
        FileUtil.runOnSubThread(new Runnable() {
            ...
            public void run() {
                File newFile = null;
                Bitmap newBmp = bitmap;//readBitMap(file.getAbsolutePath());
                if (null == newBmp) {
                    message.what = FAIL;
                    message.obj = "读取图片失败";
